...
Development is AI-assisted (Codex), and this is now publicly disclosed. The `.Codex/` directory
...
3. **No direct pushes to `main`.** Every change is a branch + PR. Enforced by `.Codex/hooks/block-main-writes.sh`.
...
`.Codex/skills/sync-upstream-core/SKILL.md`.
